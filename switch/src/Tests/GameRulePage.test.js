...
describe('GameRule Page screenshot test', () => {
  test('GameRule page should render correctly', () => {
      const renderer = new ReactShallowRenderer();
      renderer.render(<GameRulePage />);
...
test("go to back page", async()=>{
  ...
  await page.goto('http://localhost:3000/game-rule');
...
test("go to game rule page", async()=>{
    ...
    await page.click('button[type=submit]');
    await page.waitFor(3000);
    await page.click("button.Gamerule-button");
  ...
  test("go to my account page", async()=>{
    ...
    await page.click('button[type=submit]');
    await page.waitFor(3000);
    await page.click("button.MyAccount-button");
